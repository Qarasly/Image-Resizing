...
with st.container(border=True):
    st.subheader("⚙️ Step 1: Image Configuration")
    ...
    with s2:
        ...
        bg_mode = st.toggle("AI Background Removal (rembg)", help="Isolates the product image.")
    with s3:
        ...
        if color_type == "Custom Color": final_color_rgb = hex_to_rgb(st.color_picker("Pick color", "#FFFFFF"))
        elif color_type == "Standard White": final_color_rgb = (255, 255, 255)
        else: final_color_rgb = "AUTO"
        ...
        try:
            ...
            if content_tab in temp_wb.sheetnames:
                ...
                if template_headers:
                    ...
                    with m2:
                        link_header_col = st.selectbox("Which column should receive the **Resized Image Link**?", ["None"] + template_headers)
...
if not GEMINI_API_KEY and selected_ai_fields:
...
if st.button("🚀 Start Production Loop") and data_to_process:
    ...
    if output_mode == "Links (Excel Sheet)":
        ...
        for i, item in enumerate(data_to_process):
            st_txt.text(f"Processing Image {i+1}/{total}: {item['sku']}")
            try:
                if item['type'] == "file":
                ...
                if selected_ai_fields and GEMINI_API_KEY and proc_img:
                    st_txt.text(f"🤖 Gemini writing metadata {i+1}/{total}: {item['sku']}")
        ...
        st.success("✅ Output successfully mapped and filled directly into your corporate template!")
        out_excel = BytesIO()
        wb.save(out_excel)
        ...
        with zipfile.ZipFile(zip_buffer, "w") as zip_f:
            for i, item in enumerate(data_to_process):
                ...
                try:
                    ...
                    if selected_ai_fields and GEMINI_API_KEY:
                        ai_outputs = generate_dynamic_content(proc_img, selected_ai_fields, mapping_context_str)
